<template>
    <section>
        ...
        <header>
            ...
            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">The Guest secret keys that point to this clan. It will only show broadcast messages (collection log, level up, loot drop, etc) from approved usernames set below. This will allow members in other clans to show there broadcasts, without seeing there regular clan chat.</h3>
        </header>
                    ...
                    <tbody>
                        <tr v-for="key in keys" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class="px-6 py-4 text-right">
                                <button type="button" data-modal-toggle="deleteGuestModal" v-on:click="pendingDelete = key" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Delete</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <div id="deleteGuestModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
            <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                ...
                <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                    ...
                    <div class="flex justify-center items-center space-x-4">
                        <button data-modal-toggle="deleteGuestModal" type="button" class="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                            No, cancel
                        </button>
                        <button v-on:click="remove()" data-modal-toggle="deleteGuestModal" type="submit" class="py-2 px-3 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-500 dark:hover:bg-red-600 dark:focus:ring-red-900">
                            Yes, I'm sure
                        </button>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        remove() {
            axios.delete('/api/clan-secret/' + this.pendingDelete.id)
                .then((response) => {
                    this.toast.success(response.data.data);
                })
                .catch((error) => {
        ...
        getSecrets() {
            axios.get('/api/clan-secret/' + this.clanid + '?guest=true')
                .then((response) => {
